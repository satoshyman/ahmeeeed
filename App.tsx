...
const App: React.FC = () => {
  ...
  const [appSettings, setAppSettings] = useState<AppSettings>(() => {
    try {
      const saved = localStorage.getItem('doge_settings_v2');
  ...
  const [userState, setUserState] = useState<UserState>(() => {
    ...
    try {
      const saved = localStorage.getItem('doge_user_v2');
  ...
  useEffect(() => {
    try {
      localStorage.setItem('doge_settings_v2', JSON.stringify(appSettings));
      localStorage.setItem('doge_user_v2', JSON.stringify(userState));
    } catch (e) {
  ...
  useEffect(() => {
    if (window.Telegram?.WebApp) {
      ...
      if (tgUser) {
        setUserState(prev => ({ ...prev, username: tgUser.first_name || 'Guest' }));
      }
    }
  }, []);
      ...
      {showAdminAuth && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-black/90 backdrop-blur-md">
          <div className="bg-slate-900 p-8 rounded-[2.5rem] w-full max-w-sm border border-slate-800 shadow-2xl text-center">
            <h3 className="text-xl font-black text-center mb-6">لوحة الإدارة</h3>
            <input 
              type="password" 
            ...
            />
            <button 
              onClick={() => { if(authPassword === appSettings.adminPassword) { setCurrentPage(Page.Admin); setShowAdminAuth(false); } else { alert('خطأ!'); } }}
              className="w-full py-4 bg-yellow-600 rounded-2xl font-black"
            >
              دخول
            </button>
            <button onClick={() => setShowAdminAuth(false)} className="w-full py-2 text-slate-500 text-sm mt-2">إغلاق</button>
          </div>
        </div>
      )}
